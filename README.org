#+title: Babashka Babooka: Writing Command Line Clojure
...
Let's look at what that means in more concrete terms. Suppose that your neighbor
has been building a ~dream-vault~ command-line application in Rust. This
application can take your dream journal, encrypt it, and lock it away in cold
storage. ~dream-vault~ has a command line interface, such that if you call
~dream-vault store entries.edn~ it will do the needful and securely store your
dreams.

It's possible to make use of ~dream-vault~ from within your Bashka project by
using the ~babashka.process~ namespace, like this:

#+begin_src clojure
(require '[babashka.process :as bp])
(bp/shell "dream-vault store entries.edn")
#+end_src

In this snippet, we're using ~bp/shell~ to make use of ~dream-vault~'s /command
line interface/. However, if ~dream-vault~ provided a /pod interface/, we could
use it like this:

#+begin_src clojure
(require '[babashka.pods :as pods])
(pods/load-pod "dream-vault")
(require '[pod.dream-vault :as dream-vault])
(dream-vault/store "entries.edn")
#+end_src

Let's start at the last line, ~(dream-vault/store "entries.edn")~. This is the
point of pods: they expose an external process's commands as Clojure functions.
They allow these processes to have a /Clojure interface/ so that you can
interact with them by writing Clojure code, as opposed to having to shell out or
make HTTP calls or something like that.

In the next section I'll explain the first three lines of the code snippet above.

** Pod implementation

Where does the ~dream-vault/store~ function come from? Both the namespace
~pod.dream-vault~ and the functions in it are dynamically generated by the call
~(pods/load-pod "dream-vault")~.

...
We can test out this CLI spec in a REPL. There are many options for starting a
Babashka REPL, and the most straightforward is simply typing ~bb repl~ at the
command line. After that, you can paste in the code from the snippet above, then

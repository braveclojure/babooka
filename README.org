...
You enable this by adding /tasks/ to your ~bb.edn~ file:

#+begin_src clojure
{:paths ["src"]
 :tasks {add  (exec 'journal.add/add-entry)
         list (exec 'journal.list/list-entries)}}
#+end_src
...
There's a lot more to the task system that I'm not going to cover here.

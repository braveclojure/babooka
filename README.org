...
To write to the journal, we want to run the command ~./journal add "Hamsters.
Hamsters everywhere. Again."~. The result should be that a map gets appended to
the vector.
...
arguments that were passed to the script. If you created the file ~args.clj~
with the contents ~*command-line-args*~, then ran ~bb args.clj 1 2 3~, it would
print ~("1" "2" "3")~.

With this code we can add an entry by calling ~./journal "Flying!! But to IHOP??"~.
This is almost what we want; we actually want to call ~./journal add "entry text"~.
...
#+BEGIN_SRC clojure
(let [[command entry] *command-line-args*]
  (case command
    "add" (add-entry entry)))
#+END_SRC
...
- Give an intelligent error message when a user calls a command that doesn't
  ...
  add~)
- Parse arguments, converting strings to keywords, numbers, vectors, maps, etc

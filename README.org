...
/The functions on ~table~ can be simplified as ~:fn add/add-entry~ if one
destructures the arguments in the functions themselves, which I have
done. One could argue either way but I prefer the simpler ~table~ form./
